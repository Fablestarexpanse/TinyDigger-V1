...
# Grass sits between Meadow Sun and Leaf Mid, so tufts read as the ground's own grass grown up.
GRASS = target(MEADOW_SUN, LEAF_MID, 0.45)
TARGETS = {"foliage": FOLIAGE, "grass": GRASS}


def prepare(path, size, keep=0.8, green=FOLIAGE):
    ...
    leaf = solid & ~wood
    for mask, goal in ((leaf, green), (wood, WOOD)):
        if mask.any():
...
def main():
    ...
    parser.add_argument("--target", choices=sorted(TARGETS), default="foliage")
    ...
    for i, path in enumerate(args.sprites):
        ...
        atlas[y:y + cell, x:x + cell] = prepare(path, cell, green=TARGETS[args.target])
